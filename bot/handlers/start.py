...
async def menu_handler(callback_query: types.CallbackQuery):
    ...
    await callback_query.answer(chat_id=callback_query.message.chat.id,
                                text=choice)
    await callback_query.message.edit_reply_markup(inline_message_id=str(callback_query.message.message_id),
                                                   text="Please choose another option:",
                                                   reply_markup=crypto_keyboard())


@start_command_router.callback_query(lambda c: c.data in ["bitcoin", "ethereum", "bnb", "solana"])
async def crypto_choice_handler(callback_query: types.CallbackQuery):
    crypto = callback_query.data.upper()

    await callback_query.answer(chat_id=callback_query.message.chat.id,
                                text=f"You selected {crypto}!")

    await callback_query.message.edit_text(inline_message_id=str(callback_query.message.message_id),
